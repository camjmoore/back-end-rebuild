  ...
  Categories.getCategories()
    .then((cats) => {
      if (cats) {
        ...
        res
          .status(400)
          .json({ message: "The categories could not be retrieved" });
      }
    })
    .catch((error) => {
      res
        .status(500)
        .json({ error: "There was an error retrieving the categories" });
    });
});
...
router.post("/", (req, res) => {
  ...
  Categories.addCategory(newCat)
    .then((cat) => {
      res
        .status(201)
        .json({ success: "category created", id: cat[0], ...newCat });
    })
    .catch((error) => {
  ...
  Categories.editCategories(changes, id)
    .then((updated) => {
      if (updated) {
        res
          .status(200)
          .json({ success: "Category was succcessfully updated", ...changes });
      } else {
        res
          .status(500)
  ...
  Categories.deleteCategory(id)
    .then((del) => {
      if (del) {
        ...
        res
          .status(500)
          .json({ error: "There was an error deleting the category" });
      }
    })
    .catch((err) => {
