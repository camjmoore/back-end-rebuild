  ...
  Ingredients.getIngredients()
    .then((ingred) => {
      if (ingred) {
        ...
        res
          .status(400)
          .json({ message: "The ingredients could not be retrieved" });
      }
    })
    .catch((error) => {
      res
        .status(500)
        .json({ error: "There was an error retrieving the Ingredients" });
    });
});
...
router.post("/", (req, res) => {
  ...
  Ingredients.addIngredient(newIngred)
    ...
    .catch((error) => {
      res
        .status(500)
        .json({ error: "There was an error adding the Ingredient" });
    });
});
  ...
  Ingredients.editIngredient(changes, id)
    .then((updated) => {
      if (updated) {
        ...
        res
          .status(400)
          .json({ message: "The Ingredient could not be updated" });
      }
    })
    .catch((error) => {
      res
  ...
  Ingredients.deleteIngredient(id)
    .then((del) => {
      if (del) {
        ...
        res
          .status(400)
          .json({ message: "The Ingredient could not be deleted" });
      }
    })
    .catch((error) => {
      res
        .status(500)
        .json({ error: "There was an error deleting the Ingredient" });
    });
});
